...
class RAModule(nn.Module):
    ...
    def forward(self, x):
        ...
        aug_feat_x1 = (x1 * att_x1)
        aug_feat_x2 = (x2 * att_x2)
        aug_feat_x3 = (x3 * att_x3)

        att_map = torch.stack([att_weight_x1, att_weight_x2, att_weight_x3],dim=1)
        attend_panel = torch.cat([aug_x1, aug_x2, aug_x3], dim=1)
        aug_feats = torch.stack([aug_feat_x1, aug_feat_x2, aug_feat_x3], dim=1)
        
        return att_map, attend_panel, aug_feats
...
class RANet(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        _rows = rows.view(-1, 3, self.dim, self.resolution, self.resolution)
        att_maps, att_rows, aug_feats = self.attention(_rows)
        att_map = att_maps.view(batch_size, 10, 3, 1, self.resolution, self.resolution)
        aug_feat = aug_feats.view(batch_size, 10, 3, self.dim, self.resolution, self.resolution)
        ...
        return similarity, att_map, rows, aug_feat
